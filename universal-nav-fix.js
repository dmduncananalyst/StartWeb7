...
/* The words navigate to their parent pages; only the adjacent arrows open submenus. */
header.querySelectorAll('.sw7-sub-row.website>a,.sw7-sub-row.seo>a').forEach(function(link){link.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();window.location.assign(link.getAttribute('href'))})});
